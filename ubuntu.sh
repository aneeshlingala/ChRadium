...
if [ "$1" == "--device=kukui" ]; then
    cd /tmp
    wget https://github.com/hexdump0815/imagebuilder/releases/download/230218-01/chromebook_kukui-aarch64-jammy.img.gz
    gzip -d chromebook_kukui-aarch64-jammy.img.gz
    echo "Flashing, this may take a while..."
    if [[ $TARGET =~ mmcblk[01] ]]; then
            dd if=chromebook_kukui-aarch64-jammy.img of=/dev/$(echo TARGET) bs=1M
            mkdir /tmp/tmpmount
    ...
    echo "Cleaning up..."
    rm -rf /tmp/chromebook_kukui-aarch64-jammy*
    ...
    umount /tmp/tmpmount
    rm -rf /tmp/tmpmount
    echo "Script has successfully installed Ubuntu. You can now boot into the new system by inserting the drive into the Chromebook," 
